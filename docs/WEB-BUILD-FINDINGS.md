...
The generated `WebEditor` methods, grouped without omitting any public operation, are (updated 8 Aug: `moveCursorWordLeft`/`Right` and `deleteWordBackward`/`Forward` were removed with #82 — they were a second, uncalled implementation of word motion, and the chords reach the kernel through `handleKeyEvent`):
