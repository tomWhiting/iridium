 ...
 *
 * This minimal example shows how to integrate the Iridium editor
 * component into a React application with:
 * - Content management
 * - Theme switching
...
function App() {
  ...
  const [matchInfo, setMatchInfo] = useState({ count: 0, current: null as number | null });

  const webgpu = useWebGPUSupport();

  const handleSearch = () => {
    if (editorRef.current && searchQuery) {
      editorRef.current.find(searchQuery);
    }
  };
  ...
  // Show loading or error state for WebGPU check
  if (!webgpu.checked) {
    return (
    ...
    <div style={styles.container}>
      ...
      <header style={styles.header}>
        ...
        <span style={styles.subtitle}>
          GPU: {webgpu.gpuInfo?.name ?? "Unknown"} ({webgpu.gpuInfo?.backend})
        </span>
      </header>
      ...
      <div style={styles.toolbar}>
        ...
        <div style={styles.searchContainer}>
          ...
          <button style={styles.button} onClick={handlePrevMatch}>
            Prev
          </button>
          <button style={styles.button} onClick={handleNextMatch}>
            Next
          </button>
          {matchInfo.count > 0 && (
            <span style={styles.matchInfo}>
              {matchInfo.current !== null ? matchInfo.current + 1 : 0} / {matchInfo.count}
            </span>
          )}
        </div>
      </div>
