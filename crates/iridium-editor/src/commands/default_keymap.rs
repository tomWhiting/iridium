...
use super::builtin::{
    ...
    COMMENT_TOGGLE_BLOCK, COMMENT_TOGGLE_LINE, CONFIG_RELOAD, CURSOR_CHAR_LEFT,
    ...
    EDIT_DELETE_BACKWARD, EDIT_DELETE_FORWARD, EDIT_DELETE_WORD_BACKWARD, EDIT_DELETE_WORD_FORWARD,
    EDIT_INSERT_NEWLINE, EDIT_OUTDENT, EDIT_TAB, EXPLORER_TOGGLE_PANEL, EXPLORER_TOGGLE_PLACEMENT,
    HISTORY_NEXT_BRANCH, HISTORY_PREVIOUS_BRANCH, HISTORY_REDO, HISTORY_TOGGLE_PANEL, HISTORY_UNDO,
    LINES_DELETE, LINES_DUPLICATE_DOWN, LINES_DUPLICATE_UP, LINES_JOIN, LINES_MOVE_DOWN,
    LINES_MOVE_UP, MULTI_CURSOR_ADD_CURSOR_ABOVE, MULTI_CURSOR_ADD_CURSOR_BELOW,
    ...
    SEARCH_PREVIOUS_MATCH, SELECTION_COLLAPSE_TO_PRIMARY, SELECTION_SELECT_ALL, VIEW_TOGGLE_THEME,
...
pub const DEFAULT_KEYMAP_BINDING_COUNT: usize = 69;
...
const BINDINGS: &[(StrokePattern, &[StrokePattern], CommandId)] = &[
    ...
    // `Ctrl+Alt+B` moves it between a floating panel and a sidebar. `B` rather
    // than another letter on the panel shape because it is what VS Code and Zed
    // both bind the sidebar to, and this is the one chord here a hand is likely
    // to arrive already knowing.
    (
        StrokePattern::new(KeyCode::Char('b'), CTRL_ALT),
        CHORD,
    ...
    // ----- Configuration -----
    //
    // `Ctrl+Alt+R`, the fourth on this shape. It is not a toggle like the other
    // three, but it belongs to the same family in the hand: chords that change
    // what the *editor* is rather than what the document says.
    //
    // The one binding whose absence is felt most while it is missing — without
    // it, changing a key means quitting and reopening, which is the point at
    // which people stop customizing anything.
    (
        StrokePattern::new(KeyCode::Char('r'), CTRL_ALT),
        CHORD,
        CONFIG_RELOAD,
    ),
    // ----- Command palette -----
