...
| 11 | Wrap/shaping constants: `Wrap::WordOrGlyph` (cosmic default, never set), `Shaping::Advanced` (T:267, 293) | fixed today | not key members; becomes one the day `docs/SOFT-WRAP-DESIGN.md` lands `Wrap::None` — note left for that change |
| 12 | **Tab width** — how many character advances a literal tab occupies | `EditorConfig::tab_width` → `usable_tab_width` → `TextRenderer::create_buffer` (`Buffer::set_tab_width`) | `ShapeKey::tab_width`, **added 8 Aug 2026 (#86)** |

**Row 12's history, because the reasoning is the useful part.** Tab width used
to be part of row 11 — "default, never set" — and that was accurate right up
until the configuration file existed. Even then it survived a re-check, because
of a split worth stating: `EditorConfig::tab_width` was an **editing** input
only, read by `input/keyboard/behaviors.rs` for indent and unindent, and the
edits it produced moved `document_revision`, which is keyed. Nothing under
`render/` read a tab width at all.

That made it a live inconsistency rather than a dormant one — `insert_spaces =
false` is settable, and any file off disk may hold tabs regardless — so
`tab_width = 2` bought two-column *unindent* and eight-column tabs on screen.
Tom ruled to wire it through, and the cache half came with it.

⚠️ **Two places, not one.** The key member alone is insufficient: a key miss
routes through `ShapeKey::permits_line_diff`, and that path **reuses the
previous buffer**, which carries the tab width it was constructed with.
Diffing text into it leaves every tab at the old width with no line marked
dirty to say so. `permits_line_diff` therefore refuses on a tab-width change,
and `a_tab_width_change_refuses_the_line_diff` is the test that holds it —
proven red by deleting that one comparison, with three controls alongside so
