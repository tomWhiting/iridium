...
//!   second is a
//!   [`CellContent::Continuation`](crate::cell::CellContent::Continuation): it
//!   holds no character of its
...
//! It is refused and a space is written instead; see
//! [`WriteOutcome`](crate::cell::WriteOutcome) for why
//! that is the only safe answer.
//!
//! # Damage is computed, never declared
//!
//! A [`Surface`](crate::cell::Surface) holds a front buffer (what the terminal
//! is showing) and a
...
//! flicker. The single exception is
//! [`Surface::invalidate`](crate::cell::Surface::invalidate), which can only
...
//! the rule and the byte accounting behind it are on
//! [`Damage`](crate::cell::Damage).
//!
...
//! See [`ColorDepth`](crate::cell::ColorDepth) for the mapping rules.
