...
//!
//! There is no `⌘` in a terminal, so the `META_ALT` spelling the desktop panel
//! carries has no meaning here and is absent. The two tables answer the same
//! eight verbs by different chords, which is what a face-owned keymap is for.
//!
...
pub const KEYMAP_NAME: &str = "terminal-history";
...
/// panel's, this face hands no `FaceKeys` to `iridium_config` yet, so nothing
/// outside the tests has asked what the count is.
#[cfg(test)]
pub(super) const DEFAULT_BINDING_COUNT: usize = 9;
...
/// somebody noticing.
#[cfg(test)]
pub(super) const EVERY_PATTERN: &[ModifierPattern] = &[PLAIN, CTRL_ALT];
...
pub fn default_keymap() -> Keymap {
    ...
    keymap.extend(vec![
        // The ways out. `Ctrl+Alt+H` closes because it opens — and it is named
        ...
        // to.
        //
        // ⚠️ One binding covers `h` and `H` both: `StrokePattern::matches`
        // ASCII-lowercases a character keycode before comparing, so a second
        // binding on `'H'` would be dead weight rather than the extra coverage
        // the `Char('h' | 'H')` pattern it replaced looked like it was giving.
